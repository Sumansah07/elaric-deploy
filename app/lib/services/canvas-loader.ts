...
export class CanvasLoaderService {
  ...
  async loadProjectCanvas(projectId: string, maxScreens: number = 50): Promise<boolean> {
    try {
      ...
      console.log('🔍 Querying screens for project:', projectId);
      const screens = await screenPersistence.loadProjectScreens(projectId);
      console.log('📊 Screens query result:', {
        count: screens.length,
        screens: screens.map((s) => ({ id: s.id, name: s.name })),
      });

      if (screens.length === 0) {
        console.log('ℹ️ No screens found for this project - canvas will be empty');
      ...
      console.log(`📄 Found ${screens.length} screens, loading first ${screensToLoad.length}`);
      ...
      for (const screen of screensToLoad) {
        try {
          ...
          console.error(`❌ Failed to load screen ${screen.name}:`, error);
        }
      }
      ...
      if (canvasState?.activePage) {
        console.log('✅ Restored active page:', canvasState.activePage);
        canvasStore.setActivePage(canvasState.activePage);
      }

      this.currentProjectId = projectId;
      console.log(`✅ Canvas loaded successfully: ${loadedCount} screens restored`);
  ...
  async loadMostRecentCanvas(): Promise<boolean> {
    console.log('ℹ️ Canvas starts clean - screens will load as created');
